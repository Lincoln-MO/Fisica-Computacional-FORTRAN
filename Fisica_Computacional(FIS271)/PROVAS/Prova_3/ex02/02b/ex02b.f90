...
!============================================================================
	...
	integer, parameter :: nf = 10000
	integer :: k
	double precision :: xr, contNc
	real*8 :: area, erro, a, b 
	Real*8, dimension(0:nf):: x, y
...
! Inicializacao do RNG
	...
	open(100,file="azul.dat")
	open(150,file="vermelho.dat")
	a = 4.d0
	b = 1.d0
	do k=ni,nf
		call ranmar(xr)		! Utilizacao do RNG para obtrer coordenada i
		x(k)= a*xr

	end do
	do k=ni,nf
		call ranmar(xr)		! Utilizacao do RNG Para obtrer coordenada J
		y(k) = b*xr
	end do
	contNc=0.d0
	do k=ni,nf
		if (y(k) >= (b*(1.d0-(x(k)/a)**2)**(0.5d0))) then  !verifica se y esta dentro da elipse
			write(150,*) x(k), y(k)

		elseif (x(k) >= (a*(1.d0-(y(k)/b)**2.d0)**(0.5d0))) then !verifica se x esta dentro da elipse
			write(150,*) x(k), y(k)
	...
	area = 4.d0*a*b*contNc/nf ! calcula a área da elipse, proposto na letra b do ex02 da prova 
	erro = abs(area - a*b*pi)
	write(*,*) area, erro
