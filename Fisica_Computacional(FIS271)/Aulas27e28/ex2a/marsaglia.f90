...
!============================================================================
	...
	integer :: k, j, Nx, Ny, Nw
    real, parameter :: pi = acos(-1.d0)
	double precision :: x
    real ang, l
    real rx(0:nf), ry(0:nf), r(0:nf)
    

     Nx=500; Ny=500              !quantidade de pixel
     call gopen(Nx, Ny, Nw)
    !call newwindow(Nw,Nx,Ny)
     call newwindow(Nw,-2e-05,-3e-05,2.5e-05,3e-05)
	...
	open(10,file="dados1.dat")
    open(20,file="dados2.dat")
    open(30,file="dados3.dat")
    open(40,file="dados4.dat")
    open(50,file="dados5.dat")
...
do j=1,5    
    ...
    call newpencolor(Nw,j)
	do k=0,nf-1
   
		call ranmar(x)		! Utilizacao do RNG
        ang = 2*x*pi
        rx(k+1) = rx(k) +l*cos(ang)
        ry(k+1) = ry(k) + l*sin(ang)
        write(10*j,*) rx(k), ry(k)
        call drawline(nw,rx(k),ry(k),rx(k+1),ry(k+1))
        call pset(nw,rx(k),ry(k))
        call msleep(10)
    ...
    call saveimg(nw,T,-2e-05,-3e-05,2.5e-05,3e-05,'img.png'//CHAR(0),2,'convert'//CHAR(0),256)
	close(10)
    close(20)
    close(30)
    close(40)
