...
!============================================================================
    ...
    real*8, parameter :: area = acos(-1.d0) !Area esperada
	integer :: k
    real*8 erro
...
! Inicializacao do RNG
	...
	!open(1,file="dentro.dat")
    !open(2,file="fora.dat")
    
    Nc=0
    do k=ni,nf
        call ranmar(xr)                    !Utilização do RNG
        call ranmar(yr)
        if ((xr**2+yr**2)<=1) then
           ! write(1,*) xr, yr
            Nc = Nc + 1
    ...
    A = 4*(Nc/(nf*1.))
    erro = abs((area-A)/area)*100
    print*, "Area do círculo:", A
    print*, "Nc", Nc
    print*, "erro =", erro, "%"
